...
def init_session(args, local='sensor', peer='client'):
    ...
    if args.d is DH.curve25519:
        ...
        private_key = ec.generate_private_key(ec.SECP521R1(), openssl_backend)
        public_key = private_key.public_key()
...
def encrypt(key, iv, plain, args):
    ...
    encryptor = None
    if args.c is CA.aes:
        encryptor = Cipher(algorithms.AES(key), modes.CTR(iv), openssl_backend).encryptor()
    elif args.c is CA.chacha:
        encryptor = Cipher(algorithms.ChaCha20(key, iv), None, openssl_backend).encryptor()
...
def main(args):
    ...
    while not done:
        ...
        reply = requests.post('{}/putdata/client'.format(args.u), json=reply)
        code = reply.status_code
        while int(code/100) != 2 and not done:
            time.sleep(0.1)
            reply = requests.post('{}/putdata/client'.format(args.u), json=reply)
            code = reply.status_code
...
if __name__ == '__main__':
    ...
    parser.add_argument('-u', type=str, help='Server URL', default='http://192.168.1.117:5000')
